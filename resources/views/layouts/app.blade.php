...
<head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .cc-header-search input:focus {
        outline: none;
        border-color: var(--cc-green);
        box-shadow: 0 0 0 2px rgba(130,180,64,.2);
      }
      ...
      @media (min-width: 1024px) {
        .cc-buy-box { position: sticky; top: 72px; }
      }

      /* TinyMCE / product HTML body — Tailwind preflight strips defaults */
      .item-body { font-size: 15px; line-height: 1.7; color: #333; }
      .item-body h1, .item-body h2, .item-body h3, .item-body h4 {
        font-weight: 700; color: #111; margin: 1.25em 0 0.5em; line-height: 1.3;
      }
      .item-body h1 { font-size: 1.75rem; }
      .item-body h2 { font-size: 1.4rem; }
      .item-body h3 { font-size: 1.2rem; }
      .item-body h4 { font-size: 1.05rem; }
      .item-body p { margin: 0 0 1em; }
      .item-body ul, .item-body ol { margin: 0 0 1em; padding-left: 1.5em; }
      .item-body ul { list-style: disc; }
      .item-body ol { list-style: decimal; }
      .item-body li { margin: 0.25em 0; }
      .item-body a { color: var(--cc-green); text-decoration: underline; }
      .item-body strong, .item-body b { font-weight: 700; }
      .item-body em, .item-body i { font-style: italic; }
      ...
      .item-body code { background: #f3f4f6; padding: 1px 5px; border-radius: 3px; }
      .item-body pre code { background: transparent; padding: 0; color: inherit; }
      .item-body hr { border: 0; border-top: 1px solid #e5e7eb; margin: 1.5em 0; }

      .cc-search { width: 100%; height: 44px; border: 1px solid #d1d5db; border-radius: 4px; padding: 0 14px; font-size: 15px; }
      .cc-btn-primary {
        background: var(--cc-green); color: #fff; font-weight: 600; border-radius: 4px;
        padding: 10px 18px; font-size: 14px; border: 0; cursor: pointer; text-decoration: none;
...
<header class="cc-topbar sticky top-0 z-40">
    <div class="cc-container cc-topbar-inner">
        <a href="{{ route('home') }}" class="cc-logo">
            <span class="cc-logo-mark">◆</span> CodeBazaar
        </a>
        ...
        <form action="{{ route('search') }}" method="get" class="cc-header-search" role="search">
            ...
            <button type="submit" aria-label="Search">
                <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M11 18a7 7 0 100-14 7 7 0 000 14z"/></svg>
            </button>
        </form>
        ...
        <div class="cc-header-actions">
            ...
            @auth
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.dashboard') }}" class="cc-hide-mobile">Admin</a>
                @endif
    ...
    <div class="cc-drawer-panel">
        ...
        @auth
            ...
            <form method="post" action="{{ route('logout') }}">@csrf
                <button type="submit" style="width:100%;text-align:left;padding:12px 8px;border:0;border-bottom:1px solid #f1f5f9;background:none;font-weight:500">Logout</button>
            </form>
        @else
