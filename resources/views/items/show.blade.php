...
<div class="grid gap-8 lg:grid-cols-12">
  <div class="space-y-5 lg:col-span-8">
    ...
    <div class="rounded border border-slate-200 bg-white shadow-sm" id="item-tabs">
      ...
      <div class="cc-tab-panel hidden p-5 sm:p-6" data-panel="changelog">
        @forelse($changelog as $entry)
          <div class="mb-5 border-b border-slate-100 pb-5 last:mb-0 last:border-0 last:pb-0">
            <div class="flex flex-wrap items-baseline gap-2">
              <span class="text-sm font-bold text-slate-900">Version {{ $entry['version'] }}</span>
              @if(!empty($entry['date']))
                <span class="text-xs text-slate-400">{{ $entry['date'] }}</span>
              @endif
            </div>
            <div class="mt-2 whitespace-pre-line text-sm leading-relaxed text-slate-700">{{ $entry['changes'] }}</div>
          </div>
        @empty
          <p class="text-sm text-slate-500">No changelog entries yet.</p>
        @endforelse
      </div>
    ...
    <div class="cc-buy-box space-y-4">
      <div class="rounded border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <ul class="mt-4 space-y-1.5 border-t border-slate-100 pt-4 text-xs text-slate-500">
          @if($item->version)
            <li class="flex justify-between"><span>Version</span><span class="text-slate-700">{{ $item->version }}</span></li>
          @endif
