      ...
      <div class="rounded border border-slate-200 bg-white p-5 shadow-sm">
        ...
        @if(! $isFree && $extended > 0)
          <form method="post" action="{{ route('cart.add') }}" class="mt-2">
            ...
            <button class="w-full rounded border border-slate-300 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              Extended License — ${{ number_format($extended, 2) }}
            </button>
          </form>
        @endif
